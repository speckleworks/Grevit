...
namespace Grevit.Revit
{
  ...
  public static class CreateExtension
  {
    ...
    public static Element Create( this Grevit.Types.Filter filter, SpeckleGrevitBuilder host = null )
    {
      ...
      foreach ( Category category in revitDoc.Settings.Categories )
      {
        if ( filter.categories.Contains( category.Name ) || filter.categories.Count == 0 ) categories.Add( category.Id );

        FilteredElementCollector collector = new FilteredElementCollector( revitDoc ).OfCategoryId( category.Id );
      ...
      FilteredElementCollector collect = new FilteredElementCollector( revitDoc ).OfClass( typeof( ParameterFilterElement ) );
      ...
      if ( parameterFilter == null ) parameterFilter = ParameterFilterElement.Create( revitDoc, filter.name, categories );


      View view = ( View ) Utilities.GetElementByName( revitDoc, typeof( View ), filter.view );
      ...
      {
        FillPatternElement pattern = ( FillPatternElement ) Utilities.GetElementByName( revitDoc, typeof( FillPatternElement ), filter.CutFillPattern );
        filterSettings.SetCutFillPatternId( pattern.Id );
      }

      if ( filter.ProjectionFillPattern != null )
      {
        FillPatternElement pattern = ( FillPatternElement ) Utilities.GetElementByName( revitDoc, typeof( FillPatternElement ), filter.ProjectionFillPattern );
        filterSettings.SetProjectionFillPatternId( pattern.Id );
      }

      if ( filter.CutLinePattern != null )
      {
        LinePatternElement pattern = ( LinePatternElement ) Utilities.GetElementByName( revitDoc, typeof( LinePatternElement ), filter.CutLinePattern );
        filterSettings.SetCutLinePatternId( pattern.Id );
      }

      if ( filter.ProjectionLinePattern != null )
      {
        LinePatternElement pattern = ( LinePatternElement ) Utilities.GetElementByName( revitDoc, typeof( LinePatternElement ), filter.ProjectionLinePattern );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.SetCurtainPanel setCurtainPanel, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      Panel panel = ( Panel ) revitDoc.GetElement( new ElementId( setCurtainPanel.panelID ) );
      ...
      if ( wallElement.GetType() == typeof( Autodesk.Revit.DB.Wall ) )
      {
        ...
        FilteredElementCollector collector = new FilteredElementCollector( revitDoc ).OfClass( typeof( PanelType ) );
    ...
    /// <returns></returns>
    public static Element Create( this Familyinstance familyInstance, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      switch ( stype )
      {
        ...
        case Autodesk.Revit.DB.Structure.StructuralType.Brace:
          familySymbolElement = revitDoc.GetElementByName( typeof( FamilySymbol ), familyInstance.FamilyOrStyle, familyInstance.TypeOrLayer, out found, BuiltInCategory.OST_StructuralFraming );
          break;

        case Autodesk.Revit.DB.Structure.StructuralType.Column:
          familySymbolElement = revitDoc.GetElementByName( typeof( FamilySymbol ), familyInstance.FamilyOrStyle, familyInstance.TypeOrLayer, out found, BuiltInCategory.OST_StructuralColumns );
          break;

        default:
          familySymbolElement = revitDoc.GetElementByName( typeof( FamilySymbol ), familyInstance.FamilyOrStyle, familyInstance.TypeOrLayer, out found );
          break;
      }
      ...
      if ( familySymbolElement != null )
      {
        ...
        if ( familyInstance.view == null || familyInstance.view == string.Empty )
          referenceElement = revitDoc.GetLevelByName( familyInstance.level, elevation );
        else
          referenceElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.View ), familyInstance.view );
        ...
        else
        {
          ...
          {
            if ( familyInstance.points.Count == 1 ) newFamilyInstance = revitDoc.Create.NewFamilyInstance( familyInstance.points[ 0 ].ToXYZ(), familySymbol, stype );
          }
          else
          {
            if ( familyInstance.points.Count == 1 )
            {
              if ( referenceElement.GetType() == typeof( Autodesk.Revit.DB.View ) )
                newFamilyInstance = revitDoc.Create.NewFamilyInstance( familyInstance.points[ 0 ].ToXYZ(), familySymbol, ( Autodesk.Revit.DB.View ) referenceElement );
              else
                newFamilyInstance = revitDoc.Create.NewFamilyInstance( familyInstance.points[ 0 ].ToXYZ(), familySymbol, ( Autodesk.Revit.DB.Level ) referenceElement, stype );
            }
            else if ( familyInstance.points.Count == 2 )
            {
              ...
              if ( referenceElement.GetType() == typeof( Autodesk.Revit.DB.View ) )
                newFamilyInstance = revitDoc.Create.NewFamilyInstance( c, familySymbol, ( Autodesk.Revit.DB.View ) referenceElement );
              else
                newFamilyInstance = revitDoc.Create.NewFamilyInstance( c, familySymbol, ( Autodesk.Revit.DB.Level ) referenceElement, stype );
            }
          }
        }
      }
    ...
    /// <returns></returns>
    public static Element Create( this Familyinstance familyInstance, Element hostElement, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      bool found = false;
      Element familySymbolElement = revitDoc.GetElementByName( typeof( FamilySymbol ), familyInstance.FamilyOrStyle, familyInstance.TypeOrLayer, out found );
      ...
      if ( familySymbolElement != null && hostElement != null )
      {
        ...
        Autodesk.Revit.DB.Level level = ( Autodesk.Revit.DB.Level ) revitDoc.GetLevelByName( familyInstance.level, elevation );
        ...
        {
          if ( familyInstance.points.Count == 1 )
            newFamilyInstance = revitDoc.Create.NewFamilyInstance( familyInstance.points[ 0 ].ToXYZ(), familySymbol, hostElement, level, structuralType );
        }
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.RevitLine line, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      foreach ( Curve c in Utilities.GrevitCurvesToRevitCurves( line.curve ) )
      {
        if ( line.isModelCurve )
          revitDoc.Create.NewModelCurve( c, Utilities.NewSketchPlaneFromCurve( revitDoc, c ) );

        if ( line.isDetailCurve )
          revitDoc.Create.NewDetailCurve( revitDoc.ActiveView, c );

        if ( line.isRoomBounding )
        {
          CurveArray tmpca = new CurveArray();
          tmpca.Append( c );
          revitDoc.Create.NewRoomBoundaryLines( Utilities.NewSketchPlaneFromCurve( revitDoc, c ), tmpca, revitDoc.ActiveView );
        }
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.SimpleExtrusion extrusion, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      Document familyDocument = revitDoc.Application.NewFamilyDocument( ConceptualMassTemplatePath );
      ...
      Family family = null;
      revitDoc.LoadFamily( filename, out family );

      // Get the first family symbol
      FamilySymbol symbol = null;
      foreach ( ElementId s in family.GetFamilySymbolIds() )
      {
        symbol = ( FamilySymbol ) revitDoc.GetElement( s );
        break;
      }
      ...
      FamilyInstance familyInstance = revitDoc.Create.NewFamilyInstance( new XYZ( 0, 0, 0 ), symbol, Autodesk.Revit.DB.Structure.StructuralType.NonStructural );

      // Activate Mass visibility
      revitDoc.SetMassVisibilityOn();
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Level level, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      // Get all Levels from the document
      FilteredElementCollector sollector = new FilteredElementCollector( revitDoc ).OfClass( typeof( Autodesk.Revit.DB.Level ) );
      ...
      if ( level.addView )
      {
        ...
        IEnumerable<ViewFamilyType> viewFamilyTypes = from elem in new FilteredElementCollector( revitDoc ).OfClass( typeof( ViewFamilyType ) )
                                                      let type = elem as ViewFamilyType
                                                      where type.ViewFamily == ViewFamily.FloorPlan
                                                      select type;

        // Create a new view
        ViewPlan.Create( revitDoc, viewFamilyTypes.First().Id, newLevel.Id );
      }
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Column column, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      Element familyElement = revitDoc.GetElementByName( typeof( FamilySymbol ), column.FamilyOrStyle, column.TypeOrLayer, out found, BuiltInCategory.OST_Columns );
      if ( familyElement == null )
        familyElement = revitDoc.GetElementByName( typeof( FamilySymbol ), column.FamilyOrStyle, column.TypeOrLayer, out found, BuiltInCategory.OST_StructuralColumns );
      Element levelElement = revitDoc.GetLevelByName( column.levelbottom, lower.Z );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.ReferencePlane plane, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      // Get the supposed view element
      Element view = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.View ), plane.View );
      ...
      if ( view != null )
      {
        ...
        // Create a new plane
        Autodesk.Revit.DB.ReferencePlane newPlane = revitDoc.Create.NewReferencePlane2( plane.EndA.ToXYZ(), plane.EndB.ToXYZ(), plane.cutVector.ToXYZ(), v );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Grid grid, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      //#if (Revit2017 || Revit2018|| Revit2019)
      Autodesk.Revit.DB.Grid gridline = Autodesk.Revit.DB.Grid.Create( revitDoc, Autodesk.Revit.DB.Line.CreateBound( grid.from.ToXYZ(), grid.to.ToXYZ() ) );
      //#else
      // Create a new gridline
      //            Autodesk.Revit.DB.Grid gridline = revitDoc.Create.NewGrid(Autodesk.Revit.DB.Line.CreateBound(grid.from.ToXYZ(), grid.to.ToXYZ()));
      //#endif
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Hatch hatch, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      // Get all Filled region types
      FilteredElementCollector collector = new FilteredElementCollector( revitDoc ).OfClass( typeof( Autodesk.Revit.DB.FilledRegionType ) );

      // Get the View to place the hatch on
      Element viewElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.View ), hatch.view );
      ...
      FillPatternElement fillPatternElement = FillPatternElement.GetFillPatternElementByName( revitDoc, FillPatternTarget.Drafting, patternname );
      ...
      return FilledRegion.Create( revitDoc, filledRegionType.Id, viewElement.Id, listOfCurves );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.SpotCoordinate spotCoordinate, Element reference, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      // get View to place the spot coordinate on
      Element viewElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.View ), spotCoordinate.view );
      if ( viewElement != null )
      {
        ...
        if ( spotCoordinate.isElevation )
          return revitDoc.Create.NewSpotElevation( view, new Reference( reference ), spotCoordinate.locationPoint.ToXYZ(), spotCoordinate.bendPoint.ToXYZ(), spotCoordinate.endPoint.ToXYZ(), pointref, spotCoordinate.hasLeader );
        else
          return revitDoc.Create.NewSpotCoordinate( view, new Reference( reference ), spotCoordinate.locationPoint.ToXYZ(), spotCoordinate.bendPoint.ToXYZ(), spotCoordinate.endPoint.ToXYZ(), pointref, spotCoordinate.hasLeader );
      }
      return null;
    }
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Topography topography, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      return Autodesk.Revit.DB.Architecture.TopographySurface.Create( revitDoc, points );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.TextNote textnote, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;

      // Get the View Element to place the note on
      Element viewElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.View ), textnote.view );
...
#if ( !Revit2015 )
        Autodesk.Revit.DB.TextNoteType type = ( Autodesk.Revit.DB.TextNoteType ) new FilteredElementCollector( revitDoc ).OfClass( typeof( Autodesk.Revit.DB.TextNoteType ) ).FirstOrDefault();
        return Autodesk.Revit.DB.TextNote.Create( revitDoc, view.Id, textnote.location.ToXYZ(), textnote.text, type.Id );
#else
                return revitDoc.Create.NewTextNote(view, textnote.location.ToXYZ(), XYZ.BasisX, XYZ.BasisY, width, TextAlignFlags.TEF_ALIGN_LEFT, textnote.text);
#endif
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Slab slab, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      FloorType type = ( FloorType ) revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.FloorType ), slab.TypeOrLayer );
      ...
      Utilities.SortCurvesContiguous( revitDoc.Application.Create, curves );
      ...
      Element levelElement = revitDoc.GetLevelByName( slab.levelbottom, slopePointBottom.Z );
      if ( levelElement != null )
      {
        // Create a new slab
        return revitDoc.Create.NewFloor( outlineCurveArray, type, ( Autodesk.Revit.DB.Level ) levelElement, slab.structural );
      }
    ...
    public static Element Create( this Grevit.Types.Roof roof, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      RoofType type = ( RoofType ) revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.RoofType ), roof.TypeOrLayer );
      ...
      if ( type != null )
      {
        ...
        // get the supposed level
        Element levelElement = revitDoc.GetLevelByName( roof.levelbottom, slopePointBottom.Z );
        if ( levelElement != null )
        {
          ...
          else
          {
            ...
            // Create a new slab
            rvtRoof = revitDoc.Create.NewFootPrintRoof( outlineCurveArray,
    ...
    /// <param name="w"></param>
    /// <returns></returns>
    public static Element Create( this Grevit.Types.WallProfileBased w, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      // Get Wall Type
      Element wallTypeElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.WallType ), w.TypeOrLayer );

      // Get Level
      Autodesk.Revit.DB.Level levelElement = ( Autodesk.Revit.DB.Level ) revitDoc.GetLevelByName( w.level, elevation );
      ...
      if ( wallTypeElement != null && levelElement != null )
      {
        ...
        wall = Autodesk.Revit.DB.Wall.Create( revitDoc, curves, wallTypeElement.Id, levelElement.Id, true );
    ...
    /// <param name="hostElement"></param>
    /// <returns></returns>
    public static Element Create( this Grevit.Types.CurtainGridLine curtainGridLine, Element hostElement, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
    ...
    /// <returns></returns>
    public static Element Create( this Room room, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      // Get the rooms phase
      Phase phase = null;
      foreach ( Phase p in revitDoc.Phases ) if ( p.Name == room.phase ) phase = p;

      if ( phase != null )
      {
        // Create a new Room
        Autodesk.Revit.DB.Architecture.Room newRoom = revitDoc.Create.NewRoom( phase );
    ...
    /// <returns></returns>
    public static Element Create( this Adaptive adaptive, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      // Get the family Symbol
      bool found = false;
      Element faimlyElement = revitDoc.GetElementByName( typeof( FamilySymbol ), adaptive.FamilyOrStyle, adaptive.TypeOrLayer, out found );
      ...
      if ( faimlySymbol != null )
      {
        ...
        for ( int i = 0; i < ids.Count; i++ )
        {
          ...
          ElementId id = ids[ i ];
          Element em = revitDoc.GetElement( id );
    ...
    /// <returns></returns>
    public static Element Create( this Grevit.Types.Wall grevitWall, Grevit.Types.Point from = null, Grevit.Types.Point to = null, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      // Get the Wall type and the level
      Element wallTypeElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.WallType ), grevitWall.TypeOrLayer );


      if ( wallTypeElement != null && baselineCurve != null )
      {
        Autodesk.Revit.DB.Level levelElement = ( Autodesk.Revit.DB.Level ) revitDoc.GetLevelByName( grevitWall.levelbottom, baselineCurve.GetEndPoint( 0 ).Z );
        ...
        if ( GrevitBuildModel.existing_Elements.ContainsKey( grevitWall.GID ) )
        {
          ...
          wall = Autodesk.Revit.DB.Wall.Create( revitDoc, baselineCurve, wallTypeElement.Id, levelElement.Id, grevitWall.height, 0, false, true );
    ...
    public static Element Create( this Grevit.Types.FaceWall faceWall, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      Element wallTypeElement = revitDoc.GetElementByName( typeof( Autodesk.Revit.DB.WallType ), faceWall.TypeOrLayer );
      ...
      var fw = Autodesk.Revit.DB.FaceWall.Create( revitDoc, wallTypeElement.Id, loc, reference );
      return fw;
    }

    public static Element Create( this Grevit.Types.SelectionSet selection, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      if ( filter == null || filter.Name != selection.Name )
        filter = Autodesk.Revit.DB.SelectionFilterElement.Create( revitDoc, selection.Name );
    ...
    public static Element Create( this Grevit.Types.Shaft shaft, SpeckleGrevitBuilder host = null )
    {
      // to maintain backwards compatibility when this method is called from the old grevit build command
      var revitDoc = host == null ? GrevitBuildModel.document : host.RevitDoc;
      ...
      var bottom = Utilities.GetLevelByName( revitDoc, shaft.levelbottom, 0 );
      var top = Utilities.GetLevelByName( revitDoc, shaft.leveltop, 1000 );

      var opening = revitDoc.Create.NewOpening( bottom, top, outlineCurveArray );
      return opening;
    }
